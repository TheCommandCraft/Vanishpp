...
class MigrationTest {
    ...
    void testFreshInstallCreatesDefaultConfig() {
        ...
        assertEquals(4, cm.getConfig().getInt("config-version"));
    ...
    void testV1MigrationRefactorsKeys() throws IOException {
        ...
        // We need to do this BEFORE loading the plugin, but MockBukkit loads plugin from disk?
        ...
        // We can intercept the load or just manually verify the logic via MigrationManager directly if possible.
        // But better to let the plugin load normally.
        // To inject files before load with specific data folder in MockBukkit is tricky without a descriptor.
        // Instead, let's manually trigger migration or use internal file manipulation if possible.
        
        // Actually, MockBukkit.load(Vanishpp.class) sets up the data folder.
        // We can access it via configuration logic? No, we need to write the file before the plugin 'onEnable' runs or 'load' runs.
        // But MockBukkit.load calls onEnable.
        
        // Strategy: MockBukkit doesn't easily let us prepopulate the data folder before load when using load(Class).
        ...
        // Let's test MigrationManager logic directly first if possible, or try to hack the data folder.
        
        // Let's try to pass parameters or just rely on MigrationManager unit test interacting with files.
    }
    
    // Changing strategy to test MigrationManager directly to avoid MockBukkit lifecycle complexity for file prep.
    
    @Test
    void testDirectMigrationV1toV4(@TempDir Path tempDir) throws IOException {
        // 1. Setup Environment
        ...
        // We need to inject the data folder behavior into the plugin mock if MigrationManager uses it for backups
        ...
        // We need to ensure plugin.saveResource works. MockBukkit supports this if we have the resource in src/main/resources.
        // Since we are running in the project, it should find it.
        
        mm.runMigration(configFile, 1, 4);
    ...
    @Test
    void testDirectMigrationV3toV4_DeepMerge(@TempDir Path tempDir) throws IOException {
         // 1. Setup Environment
        ...
        // Since we already loaded the plugin, it created a default config or used existing. 
        // Let's force run the migration method on this file.
        ConfigManager cm = plugin.getConfigManager();
        new MigrationManager(plugin, cm).runMigration(configFile, 3, 4);
        ...
        // If config.yml in src/main/resources has something new, it should be in result.
        // Let's assume 'messages.staff-notify.enabled' is new or we just check a standard key
